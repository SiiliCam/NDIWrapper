...
int main() {
  ...
  receiver.addMetadataCallback([](MetadataContainer container) -> void {
    ...
    if (container.aspectRatio.has_value()) {
      ...
      Logger::log_info("RECEIVER got aspect ratio height:", aspectRatio.height,
                       "width", aspectRatio.width);
    }
    if (container.boundingBox.has_value()) {
      const auto &boundingBox = container.boundingBox.value();
      Logger::log_info("RECEIVER got bounding box, start x",
                       boundingBox.start.x, "start y", boundingBox.start.y,
                       "width", boundingBox.width, "height", boundingBox.width);
    }
    if (container.switchCamera.has_value()) {
      const auto &switchCamera = container.switchCamera.value();
      Logger::log_info("RECEIVER got switchCamera: ",
                       container.switchCamera.value());
    }
  });
  ...
  for (int i = 0; i < 20; i++) {
    NDISender *sender =
        new NDISender{"mymdnsname" + std::to_string(i), "Testing", true, true};
  ...
  sender.addMetadataCallback([](MetadataContainer container) -> void {
    ...
    if (container.aspectRatio.has_value()) {
      const auto &aspectRatio = container.aspectRatio.value();
      Logger::log_info("SENDER got aspect ratio height:", aspectRatio.height,
                       "width", aspectRatio.width);
    }
    if (container.boundingBox.has_value()) {
      const auto &boundingBox = container.boundingBox.value();
      Logger::log_info("SENDER got bounding box, start x", boundingBox.start.x,
                       "start y", boundingBox.start.y, "width",
                       boundingBox.width, "height", boundingBox.width);
    }
    if (container.switchCamera.has_value()) {
      const auto &switchCamera = container.switchCamera.value();
      Logger::log_info("SENDER got switchCamera: ",
                       container.switchCamera.value());
    }
  });
  ...
  std::thread t([&]() -> void {
    ...
    while (true) {
      ...
      if (!frame.data.empty()) {
        ...
        Logger::log_info("red median:", redMedian, "green median:", greenMedian,
                         "blue median:", blueMedian, "image width", frame.width,
  ...
  receiver.addAudioCallback([](Audio audio) {
    ...
    Logger::log_info(
        "datalength:", audio.data.size(), "total:", total,
        "mean:", std::to_string(total / static_cast<float>(audio.noSamples)),
        "number of samples:", audio.noSamples,
        "channels:", static_cast<int>(audio.channels),
